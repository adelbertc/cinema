...
object PairwiseApp {
  def calculate(myGraph: Graph, k: Int, metric: (Graph, Int, Int) => Double, outputFilename: String, numOfServers: Int) {
    val system = ActorSystem("PairwiseApp")
    ...
    val producer = system.actorOf(Props(new PairwiseProducer(myGraph, slices.toList, vertexSubset, metric, outputFilename)), name = "graphproducer")
