...
class SingleSourceDirector(myGraph: Graph, metric: (Graph, Int) => Map[Int, Int]) extends Director {
  def receive = {
...
object SingleSourceProducer {
  def props(myGraph: Graph, metric: (Graph, Int) => Map[Int, Int]) = Props(new SingleSourceDirector(myGraph, metric)).withRouter(FromConfig())
}

class SingleSourceProducer(myGraph: Graph, slices: List[Vector[Int]], subset: Vector[Int], metric: (Graph, Int) => Map[Int, Int], outputFilename: String) extends Producer {
